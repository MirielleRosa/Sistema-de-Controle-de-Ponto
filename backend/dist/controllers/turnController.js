...
class TurnController {
    constructor(turnService) {
        ...
        this.getTodayWorkedHoursHistory = (_a, res_1) => __awaiter(this, [_a, res_1], void 0, function* ({ params: { userId } }, res) {
            try {
                console.log(`Procurando histórico de horas para o usuário ${userId}`);
                const history = yield this.turnService.getTodayWorkedHoursHistory(Number(userId));
                res.status(200).json(history);
            }
            catch (error) {
                this.handleError(res, 'Erro ao carregar histórico de horas trabalhadas', error);
        ...
        this.getWorkedHoursToday = (req, res) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const today = yield this.turnService.getWorkedHoursToday(Number(userId));
